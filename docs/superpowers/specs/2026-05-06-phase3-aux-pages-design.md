# Phase 3 â€” Aux Pages (Reviews + Contact + Kickstart polish) Design Spec

**Date:** 2026-05-06
**Source brief:** `GB_Whittier_Website_Build_Brief (1).docx` (client-supplied, May 2026), Part 3 (Page-by-Page Copy & Design Guide)
**Phase:** 3 of 4 in the Brief Implementation roadmap (offer alignment âœ… â†’ program pages âœ… â†’ **aux pages** â†’ SEO/tracking)
...
The bento-card visual language locked in Phase 1 propagates here. Reviews and Contact get full rewrites. Kickstart gets a minor polish pass â€” its structure already matches the brief (it's the funnel page with no nav/footer per brief Part 5), so this phase only verifies brief alignment and tightens copy where deviations exist.

The `/congrats` page is explicitly deferred â€” the brief Part 1 site architecture says "Congratulations Page (Build later)" and we honor that. Phase 4 may revisit it as part of the SEO/tracking infrastructure pass.

## Goals

- `src/pages/reviews.astro` rewritten to brief Part 3 (Reviews page) verbatim: minimal hero, reviews widget section, 3â€“4 featured quote cards, final CTA. Bento-card visual, semantic H1/H2, BreadcrumbList JSON-LD, brief-aligned meta.
...
- `src/pages/kickstart.astro` verified against brief Part 3 (Kickstart) â€” adjust copy where it deviates from the brief, but do NOT restructure (it's a funnel page; minimal layout is intentional).
...
- Form/calendar embed swaps â€” webhook integration is the contract
...
| File | Change |
|---|---|
| `src/pages/reviews.astro` | Full rewrite â€” bento visual, hero H1/H2 fix, brief-aligned content, CTA labels, BreadcrumbList |
| `src/pages/contact.astro` | Full rewrite â€” bento visual, hero H1/H2 fix, brief-aligned NAP/form/map/service-areas, BreadcrumbList |
| `src/pages/kickstart.astro` | Targeted edits â€” verify brief copy alignment, tighten CTA labels if any deviate |

Approximate diff: 3 files. No new components, no new dependencies. Reuses `BaseLayout`, `CTAButton`, `OptInForm` (not embedded on these pages â€” no homepage form duplication needed since these are not direct conversion pages), `SchemaBreadcrumb`, `SchemaLocalBusiness`. Note: Reviews and Contact pages link to homepage `/#trial` for conversion (these are research-and-trust pages, not conversion-funnel-entry pages).
...
- Heading stack on every page: SEO label `<p>` + visible small `<h1>` + display `<h2>` (where applicable â€” Contact may use H1 only without a display H2 since it's an info page, not a sales page).
...
1. **Hero (rounded card)**
   - SEO label `<p>`: `Reviews â€” Gracie Barra Whittier`
   - `<h1 class="text-base md:text-lg font-semibold text-gb-white/90 mb-3">`: `What Students Say About Gracie Barra Whittier`
   - `<h2 class="text-3xl ... font-extrabold ...">`: same line as H1 styled larger? **No** â€” Reviews page in brief uses single H1 with no display H2. Drop the `<h2>` slot and let the H1 be the visible main heading at full display size. Use `<h1 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl ...">`. Brief says only one H1 with the keyword phrase; no separate motivational display line for this page.
   - Subheadline `<p>`: `Real reviews from real families in Whittier, La Habra, La Mirada, and Pico Rivera.`
   - Primary CTA: `Start My Free Trial` â†’ `/#trial` (links back to homepage opt-in form per brief)

2. **Reviews widget section** (white card)
   - `<h2>`: `Live Google Reviews` (sr-only acceptable; visible heading optional â€” if visible, use it for SEO)
   - Above the widget: small text `Powered by Google Reviews â€” updated in real time`
   - Widget container `<div id="reviews-widget">` â€” placeholder div for the GHL/LocalCraze widget. Brief says "Currently shows 0 reviews. Will populate when Firestorm campaign runs." We render the placeholder element; the GHL embed script (mounted in BaseLayout via `PUBLIC_REVIEWS_EMBED_ID`) populates it at runtime.
...
3. **Featured Review Quotes** (white card section with grid)
   ...
   - 3â€“4 quote cards in `grid sm:grid-cols-2 lg:grid-cols-4` (or `lg:grid-cols-3` for 3 quotes). Each card: blockquote, name, optional role.
...
4. **Final CTA strip** (full-width navy band)
   ...
   - Trust line: `3 free classes â€¢ Free uniform rental â€¢ No contracts`

5. **`<SchemaBreadcrumb>`** â€” head emit, items `[{Home, /}, {Reviews, /reviews/}]`
...
1. **Hero (rounded card, smaller than program-page heroes â€” Contact is a utility page)**
   - SEO label `<p>`: `Contact Gracie Barra Whittier`
   - `<h1>` (full display size â€” no separate display H2): `Contact Gracie Barra Whittier`
...
2. **NAP Block + Hours** (white card section, two-column on desktop)
   - Left column: `<h2>` `Visit, Call, or Email`. Address block (in plain HTML, NOT image â€” brief Part 3 explicitly notes this for SEO), phone (tel link), email (mailto link). All NAP from `src/content/nap.ts`.
...
3. **Contact Form** (white card section)
   ...
   - Submit button: `Send Message` (brief explicit â€” this stays despite the global CTA-ban list because the brief mandates it)
   ...
   - On success: render an inline confirmation message ("Message received. We'll reply within 1 business day."). NO redirect â€” Contact page is not a funnel.
...
5. **Service Areas** (white card section)
   ...
   - Body: brief verbatim â€” `Gracie Barra Whittier is conveniently located on Whittier Blvd and easily accessible from Whittier, La Habra, La Mirada, and Pico Rivera.`
...
7. **`<SchemaBreadcrumb>`** â€” head emit, items `[{Home, /}, {Contact, /contact/}]`
8. **`<SchemaLocalBusiness>`** â€” head emit (Contact is the canonical business-info page; reinforces local SEO).

## Kickstart page (`src/pages/kickstart.astro`)

Targeted edits only. Brief Part 5 says: "NO NAV. NO FOOTER. THIS IS A FUNNEL PAGE â€” NO EXITS. GOAL: GET THEM TO BOOK THEIR FIRST CLASS." Current page already uses `FunnelLayout` (no Nav/Footer). Verify and tighten:

### Brief-required content checks (verify each in current file; edit if missing or off-spec)

1. **Confirmation header** â€” `<h1>You're In, [First Name]! Your Free 3-Class Pass Is Reserved.` (already aligned in Phase 1 commit `b9474e5`).
2. **Age routing note** â€” paragraph `Not sure which program to choose?` plus the 5-bullet age-to-program map (Tiny 3-4, LC1 5-6, LC2 7-9, Juniors 10-15, Adults 16+). Already present.
3. **GHL Calendar embed** â€” replaced by our custom booking calendar (out of scope; webhook integration is the contract).
4. **What Happens Next** â€” 3-step visual:
   - Book your first class
   - Show up â€” wear workout clothes, we provide the uniform
   - After class, meet with Alex â€” no pressure, no commitment
5. **Trust strip** â€” small horizontal: `Free uniform rental included â€¢ No contracts â€¢ No pressure â€¢ World-class Gracie Barra curriculum`
...
- Do NOT add bento wrapper, do NOT add full-width navy bands â€” funnel page minimal aesthetic stays.
- Do NOT add BreadcrumbList (funnel page â€” no schema, no nav signal).
...
- Reviews: 3â€“4 featured quote cards, reviews-widget placeholder div, brief-aligned subheadline.
...
- BreadcrumbList JSON-LD on `/reviews/` â€” 2 ListItem entries.
- BreadcrumbList JSON-LD on `/contact/` â€” 2 ListItem entries.
- LocalBusiness JSON-LD on `/contact/` â€” 1 entry (in addition to homepage's).

**Grep checks (must return zero matches in `src/pages/{reviews,contact,kickstart}.astro`):**
- `Get My Free Class`
- `Free First Class`
- `Click Here`
- `>Submit<` and `label="Submit"` (brief allows `Send Message` and the canonical CTA labels â€” `Submit` is banned)
- `Get Started` not followed by ` Free`
- `#trial-form` (legacy dead anchor)

**Automated:**
- `npx astro check` â†’ 0 errors, 0 warnings
- `npx vitest run` â†’ all tests pass (Phases 1 and 2)
...
**Inputs:** brief extracted text, this spec, codebase root, Vercel preview URL, scope = "Phase 3 only â€” Reviews/Contact/Kickstart per brief Part 3".
...
| Reviews widget script (`PUBLIC_REVIEWS_EMBED_ID`) doesn't populate at runtime, leaving the placeholder text visible | Low | Low | Acceptable â€” brief explicitly notes "Currently shows 0 reviews. Will populate when Firestorm campaign runs." Visual fallback text is brief-acceptable. |
...
| Kickstart page changes break the booking flow | Low | High | Phase 3 only edits Kickstart copy/labels â€” does not touch `<BookingFlow>`, `/api/book`, or `/api/availability`. Manual smoke check after deploy. |
| Phase 1's strict CTA-label clause conflicts with `Send Message` button on Contact form | Resolved | â€” | Brief Part 3 (Contact Â§2) explicitly mandates `Send Message`. Phase 3 spec overrides Phase 1's banned-list for this single label. Audit must not flag `Send Message` as a violation. |
